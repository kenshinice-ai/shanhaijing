...
**监控**：`.github/workflows/uptime.yml` 每 6 小时探一次生产站点，检查五件事——首页返回 200、
内容确为本图集（不是被停放页顶替）、数据载荷非空、无背书声明仍在产物中、未命中路径仍返回 404。
任一失败即 workflow 失败，GitHub 通知仓库所有者。

**它不是什么**：6 小时粒度不是 SLA 级监控，没有响应时间趋势，也没有多地探测。
「首页 200 但图集是空的」这类故障它能抓（所以才检查载荷非空）；「刚刚宕了五分钟」抓不到。
要更细的粒度得接第三方探针，属待办。
