...
## 4.5 校勘执行者资质与免责（2026-08-19 增设）

本项目的校勘裁决**并非由古籍专业人员作出**。《西山经》语料的逐条裁决由 AI 执行、
由项目所有者授权，双方均不具备版本学专业资质。这一节把由此而来的限制写死，
以免它被后来的读者、引用者或本项目自己遗忘。

### 4.5.1 允许采信的依据

裁决只能建立在以下四类依据之上，且必须逐条写明用的是哪一类：

| 依据 | 含义 | 例 |
|---|---|---|
| `formulaic` | 全书或本篇的固定句式 | 「其狀如…其名曰…」「見則…」「X流注于Y」 |
| `internal_parallel` | 同书他处同类文例 | 段38 与段81 同作「白菅為席」 |
| `character_sense` | 二字义别明确且上下文只容一义 | 「飢」谓腹饿，「饑」谓年荒 |
| `defective_variant` | 一侧文句不成立 | 「其陽…其陽」自相抵牾、「于於」连用 |
| `arithmetic` | 原文自带的数字可作检验 | 山系结语的里程与山数 |

### 4.5.2 禁止采信的依据

- **「通行本作」「一般认为」「学界多从」之类无出处的表述，一律不得作为裁决依据。**
  执行者没有版本学训练，此类说法在其笔下只是转述记忆，不是证据。
- 不得以「读起来更顺」「更符合现代语感」定案。
- 不得因为某读法更便于建模、检索或展示而采用它。

### 4.5.3 三档结论，不是两档

裁决结果分三档，**档位本身就是信息**，不得混同：

| 档位 | 含义 | 对 baseline 的处置 |
|---|---|---|
| `keep_base` / `adopt_cross` | 内部证据足以定案 | 采定案读法 |
| `provisional` | 证据有倾向但不足以定案 | 取倾向读法作**工作底本**，置信度记 `low`，另一读留档 |
| `unresolved` | 无倾向可言 | 保留底本原样，写明需要什么才能定案 |

设 `provisional` 一档，是因为**古书本难求全书自洽**——本篇四列山系的段内里程相加与
各自结语即差 −140／+530／−304／−95，这不是校勘能消除的，是文本自身的状态。
把这类差异无限期挂在「未决」，既不会让文本更可靠，也让读者无从使用；
取内部证据较优者作工作底本、同时把不确定性明码标出，是更诚实的处理。

**但 `provisional` 绝不可上升为定案**：专家复核应当优先重看这一档。
截至 2026-08-19：**定案 35（沿用底本 23 + 采校核本 12）、暂定 10、未决 0**，另有 45 处异体差异依 X-2 丢弃（不入 variant）。（此前本行记作「定案 23 处、暂定 10 处、未决 0 处」，只数了 `keep_base` 而漏了 `adopt_cross`。计数以 `npm run collate:xishan` 的输出为准——它数的是实际应用的差异；直接数 `xishan_rulings_v1.json` 的 `signatures` 会多算，因为段16、段19 各有两种等价切分写在同一条裁决里。）

### 4.5.3a 数字类差异的特别说明

里程与山数的差异**不以「能否弥合结语」为定案标准**。结语本身就与段内相加不符，
拿一个对不上的总数去校准个别数字，是把噪声当基准。数字差异仍按内部证据（句式频次、
该句在两本中是否另有讹脱）判断，且一律不高于 `provisional`，除非有句式上的硬证据。

### 4.5.4 对外披露义务

凡对外呈现由本项目校勘的文本，必须同时可见「校勘未经古籍专家复核」的声明。
站点上的无背书声明承担此义务；引用本项目文本者应一并转述该限制。

### 4.5.5 可推翻性

每条裁决都附依据、置信度与裁决人，载于 `scripts/data/xishan_rulings_v1.json`。
专家复核推翻其中任何一条，都属**正常修订而非事故**：改动裁决表并重跑
`npm run collate:xishan`，语料与 checksum 随之更新，无需重新抓取底本。
